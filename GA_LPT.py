...
class GAScheduler:
    ...
    def fitness(self, population_size, parent_list, offspring_list):
        ...
        # NOTE Sort jobs by processing time for SPT rule
        sorted_jobs = np.argsort(self.pt[:, 0])[
            ::-1
        ].tolist()  # Assuming self.pt[:, 0] contains processing times
        ...
        for m in range(population_size * 2):
            ...
            # Evaluate chromosome based on adherence to LPT order
            lpt_score = sum(
                [
                    1
                    for i, job in enumerate(total_chromosome[m][0])
                    if job == sorted_jobs[i]
                ]
            )
            ...
            # NOTE Incorporate SPT score into fitness
            chrom_fitness.append(1 / makespan + lpt_score)  # reciprocal of makespan
    ...
    def run_genetic(self, population_size=100, num_iteration=100, verbose=False):
