...
class GAScheduler:
    ...
    def init_population(self, population_size):
        ...
        for i in range(population_size):
            ...
            random_chromosome = [
                np.random.permutation(np.arange(1, self.num_job + 1)).tolist(), # operation sequence
                np.random.randint(1, self.num_mc + 1, size = self.num_job).tolist() # machine selection
            ]
    ...
    def crossover(self, population_size, population_list):
        ...
        def relativeSortArray(A, B, cutpoint):
            ...
            new_A[0][cutpoint:] = sorted(new_A[0][cutpoint:], key=(B + sorted(new_A[0][cutpoint:])).index)
